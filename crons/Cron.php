...
class Cron
{
    ...
    private function runOneDay(array $data): void
    {
        ...
        if ($rows !== null) {
            ...
            foreach ($rows as $row) {
                ...
                $this->db->query('UPDATE users SET money = money + ?, rmdays = GREATEST(rmdays - 1, 0), bank = IF(bank > 0, bank + ?, bank), searchdowntown = 100, slapping = 0, gamble_daily = 0 WHERE id = ?');
    ...
    {
        // Count distinct players who have entered a fighter
        $this->db->query('SELECT COUNT(DISTINCT playerid) FROM luckyboxes WHERE playerid != 0');
        $this->db->execute();
        $playerCount = (int)$this->db->result();
